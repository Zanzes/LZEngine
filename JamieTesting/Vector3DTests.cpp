...
TEST(Vector3D, projectOntox)
{
	...
	EXPECT_FLOAT_EQ(dotResult, cos(LZSoft::Math::PI / 6.0f));
...
TEST(Vector3D, lerp)
{
	...
	while(alpha <= 1.0f)
	{
		Vector3D lerpResult1 = ProMath::lerp(alpha, source, target);
